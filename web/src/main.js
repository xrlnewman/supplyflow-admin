...
function supplyWorkflowPanel() {
  ...
  return `<section class="panel full workflow-panel" style="margin-top:24px"><div class="panel-head"><div><h2>采购进度工作台</h2><p>申请 → 询价比价 → 审批 → 下单 → 收货质检 → 入库核销</p></div><span class="chip">${purchaseRequests.length} 条真实业务记录</span></div><div class="workflow-stages" style="display:flex;align-items:center;gap:10px;color:#5b5b86;font-size:12px;margin:8px 0 20px">${stages.map((stage) => `<span style="padding:7px 11px;background:#f1efff;border:1px solid #ded9ff;border-radius:999px">${stage}</span>`).join('<i>→</i>')}</div><div class="table"><div class="th"><span>申请单 / 申请人</span><span>采购原因</span><span>金额（CNY）</span><span>当前状态</span><span>下一步</span></div>${purchaseRequests.map((item) => `<div class="tr"><span><strong>${item.id}</strong><small>${item.requester}</small></span><span>${item.reason}</span><span>¥${item.estimatedAmount}</span><b class="status indigo">${item.status}</b><button class="text-action" data-workflow="supply" data-request-id="${item.id}" data-status="${item.status}">${item.status === '询价中' ? '提交审批' : item.status === '待审批' ? '生成订单' : item.status === '部分到货' ? '登记质检' : '查看详情'}</button></div>`).join('')}</div></section>`
}
...
function mobileView() {
  return `<section class="mobile-panel"><div class="mobile-panel__hero"><span>SUPPLYFLOW MOBILE</span><h2>我的采购与供应商跟进</h2><p>移动端与采购 API 共享申请、询价、到货、质检和入库状态。</p><button class="primary" data-action="create-appointment">＋ 创建演示采购单</button></div><div class="mobile-list"><h3>今日采购单</h3>${appointments.slice(0, 4).map((appointment) => `<article class="mobile-card"><div><small>${timeLabel(appointment.scheduledAt)} · ${appointment.department}</small><strong>${appointment.patient}</strong><span>${appointment.doctor} · ${appointment.status}</span></div><b class="status ${statusColors[appointment.status] || 'indigo'}">${appointment.status}</b>${appointmentAction(appointment)}</article>`).join('')}</div><div class="mobile-list"><h3>供应商跟进</h3>${followupTasks.slice(0, 3).map((item) => `<article class="mobile-card"><div><small>${item.dueAt}</small><strong>${item.summary}</strong><span>${item.patient} · ${item.status}</span></div>${item.status === '已完成' ? '<b class="status green">已完成</b>' : `<button class="text-action" data-action="complete-followup" data-followup-id="${item.id}">完成跟进</button>`}</article>`).join('')}</div></section>`
}
...
async function createAppointment() {
  try {
    const created = await api.createAppointment({ patient: '移动端演示客户', patientId: 'SUP-MOBILE-DEMO', department: '智能家电', doctor: '林然 · 采购专员', scheduledAt: new Date().toISOString() })
