...
function supplyWorkflowPanel() {
  ...
  return `<section class="panel full workflow-panel" style="margin-top:24px"><div class="panel-head"><div><h2>采购闭环工作台</h2><p>申请 → 询价比价 → 审批 → 下单 → 收货质检 → 入库核销</p></div><span class="chip">${purchaseRequests.length} 条真实业务记录</span></div><div class="workflow-stages" style="display:flex;align-items:center;gap:10px;color:#5b5b86;font-size:12px;margin:8px 0 20px">${stages.map((stage) => `<span style="padding:7px 11px;background:#f1efff;border:1px solid #ded9ff;border-radius:999px">${stage}</span>`).join('<i>→</i>')}</div><div class="table"><div class="th"><span>申请单 / 申请人</span><span>采购原因</span><span>金额（CNY）</span><span>当前状态</span><span>下一步</span></div>${purchaseRequests.map((item) => `<div class="tr"><span><strong>${item.id}</strong><small>${item.requester}</small></span><span>${item.reason}</span><span>¥${item.estimatedAmount}</span><b class="status indigo">${item.status}</b><button class="text-action" data-workflow="supply" data-request-id="${item.id}" data-status="${item.status}">${item.status === '询价中' ? '提交审批' : item.status === '待审批' ? '生成订单' : item.status === '部分到货' ? '登记质检' : '查看详情'}</button></div>`).join('')}</div></section>`
}
